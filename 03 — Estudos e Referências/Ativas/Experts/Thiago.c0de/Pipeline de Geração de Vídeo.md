---
created: 2026-04-19
updated: 2026-04-26
type: recurso
status: ativo
tags: [expert, thiago.c0de, vídeo, pipeline, workflow]
area: pesquisa
---

# Pipeline de Geração de Vídeo — Thiago.c0de

> Empilhamento de IAs para vídeos de cinema em 6 passos.

## 1. Workflow Prático
1. **Ideação (Claude):** 10 ângulos com viés "Facada no coração".
2. **Engenharia de Prompt:** Iluminação dramática + Profundidade de campo.
3. **Imagens (Flux):** Geração de cenas brutas. Seed constante para personagens.
4. **Animação (Kling AI):** Image-to-Video. Clipes de 5s. Troca de ângulo constante.
5. **Som (ElevenLabs):** Voz emocional (Respiração, pausas).
6. **Montagem (CapCut):** Sincronia SFX + Trilha.

Fundamento → [[03 — Estudos e Referências/Ativas/Experts/Thiago.c0de/Criativos Cinematográficos]] · Aplicação → [[Projetos/Holos/Holos - Formação Imersiva Natureza]]

---
- **Auditoria:** Stack técnica (Flux, Kling AI, ElevenLabs, CapCut) e regra de clipes de 5s preservados. Caveman style.

---
[[03 — Estudos e Referências/Ativas/Experts/Thiago.c0de/Criativos Cinematográficos]] | [[Projetos/Holos/Holos - Funil Natureza]] | [[_INDEX]]
